...
RESULT_FOLDER = Path("logs") / process_start_time.strftime("%Y%m%d_%H%M%S")


class LogFile(BaseModel):

    dirpath: Path = RESULT_FOLDER
    ...
    def model_post_init(self, __context: Any) -> None:
        if self.desc:
            self.dirpath = Path(f"{self.dirpath}_{self.desc}")

        if self.filename:
            self.filepath = self.dirpath / self.filename
        else:
            self.filepath = self.dirpath / "log.txt"
...
def create_logger(log_file: LogFile):

    if not os.path.exists(log_file.dirpath):
        os.makedirs(log_file.dirpath)

    file_mode = "a" if os.path.isfile(log_file.filepath) else "w"
    ...
    # write to file
    file_out = logging.FileHandler(log_file.filepath, mode=file_mode)
...
class TimeEstimator:
    ...
    def get_est(self, count, total):
        ...
        remain = total - count
        remain_time = inf
        if denominator := count - self.count_zero:
            remain_time = elapsed_time * remain / denominator
